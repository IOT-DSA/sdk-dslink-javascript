...
DSNode.prototype.toMap = function() {
  ...
  if(!_.isNull(this.driver))
    map.profile.driver = this.driver;
  if(!_.isNull(this.icon))
    map.configs.icon = this.icon;
  if(!_.isNull(this.updateInterval))
    map.configs.updateInterval = this.updateInterval;

  var type = _.merge(this.type);
  delete type.name;
  map.configs = _.merge(map.configs, type);
