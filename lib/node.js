...
_.inherits(DSNode, EventEmitter);
...
DSNode.prototype.toMap = function() {
  var map = {
    ...
    'actions': _.map(this.actions, function(action) { return this.actions[action].toMap(); })
  };
...
_.inherits(DSRecordingNode, DSNode);
...
DSAction.prototype.toMap = function() {
  ...
  var map = {
    'name': this.name,
    'parameters': _.map(this.params, function(param) { return listParams(this.params, param, false); })
  };
  if(this.results) {
    map.results = _.map(this.results, function(result) { return listParams(this.results, result, true); });
  }
