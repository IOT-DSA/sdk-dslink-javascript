...
var Rollup = Object.freeze({
  'avg': function(vals) {
  	...
  	var prims = _.map(vals, function(val) { return val.value; });
  	return new Value(prims.reduce(function(a, b) { return a + b; }) / prims.length);
  },
  'min': function(vals) {
  	return copySort(vals, function(a, b) {
  	  return Math.min(_.toNumber(a.value), _.toNumber(b.value)) === _.toNumber(a.value) ? -1 : 1;
  	})[0];
  },
  'max': function(vals) {
  	return copySort(vals, function(a, b) {
  	  return Math.max(_.toNumber(a.value), _.toNumber(b.value)) === _.toNumber(a.value) ? -1 : 1;
  	})[0];
  },
  'sum': function(vals) {
  	if(!vals.every(function(val) { return val.type.name === ValueType.NUMBER.name; })) {
