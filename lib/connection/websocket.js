...
function WebSocketClient(name, hostname) {
  HandshakeClient.call(this, name, hostname, function(data) {
    ...
    var path = _.replaceAll(_.replaceAll(this.hostname, 'https', 'wss'), 'http', 'ws') + data.wsUri + '?dsId=' + this.dsId + '&auth=' + _.Base64.urlSafe(auth.digest('base64'));
...
WebSocketClient.prototype.handleMessages = function(messages) {
