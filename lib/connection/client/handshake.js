...
function HandshakeClient(name, hostname, cb) {
  ...
  curve.generateKeyPair(NamedCurve.P256).then(function(keys) {
    ...
    crypto.sha256(keys.publicKey.getEncoded()).then(function(hash) {
      ...
      var req = (url.parse(this.hostname).protocol.indexOf('https') >= 0 ? https : http).request({
        ...
        res.on('data', function(buf) {
          ...
          this.__priv__.privateKey.getSharedSecret(curve.Point.fromEncoded(NamedCurve.P256, tempKey)).then(function(secret) {
            this.__priv__.sharedSecret = secret;
            cb(data);
          }).catch(function(e) {
            throw e;
          })
