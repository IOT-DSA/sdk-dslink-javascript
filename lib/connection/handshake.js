...
function HandshakeClient(name, hostname, cb) {
  ...
  var req = (url.parse(this.hostname).protocol.indexOf('https') >= 0 ? https : http).request({
    ...
    res.on('data', function(buf) {
      ...
      this.__priv__.nonce = this.decryptNonce(this.__priv__.privateObj, data.encryptedNonce);
...
HandshakeClient.prototype.decryptNonce = function(privateKey, encryptedNonce) {
  var exponent = bignum.fromBuffer(privateKey.getPrivateExponent()),
      modulus = bignum.fromBuffer(privateKey.getModulus()),
      nonce = bignum.fromBuffer(new Buffer(encryptedNonce, 'base64'));

  nonce = nonce.powm(exponent, modulus).toBuffer();
