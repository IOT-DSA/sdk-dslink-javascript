...
function decryptNonce(privateKey, encryptedNonce) {
  var modulus = bignum.fromBuffer(privateKey.getModulus()),
      nonce = bignum.fromBuffer(new Buffer(encryptedNonce, 'base64').slice(1)).pow(65537).mod(modulus).toBuffer();
  ...
  if(nonce.length > 16) {
    return buf.slice(nonce.length - 16);
  }
...
function HandshakeClient(name, hostname, cb) {
  ...
  var req = (url.parse(this.hostname).protocol.indexOf('https') >= 0 ? https : http).request({
    ...
    res.on('data', function(buf) {
      ...
      // this.__priv__.pollerInterval = data['min-update-interval-ms'];
      this.__priv__.pollerInterval = data.updateInterval;
      this.__priv__.salt = data.salt;
      this.__priv__.nonce = decryptNonce(this.__priv__.privateObj, this.__priv__.publicObj, data.encryptedNonce);
