...
var _ValueTrend = {
  ...
  'next': function(trend) {
    var returned = null;
    trend.__priv__.trend.values.some(function(val) {
  ...
  'toMap': function(trend) {
    var map = {
      'columns': [
        ...
        {
          "name": "value",
          "type": trend.type.name
        }
      ],
      'rows': []
    };

    map.columns[1] = _.merge(map.columns[1], trend.type.toMap(false));

    if(trend.type.name === 'number') {
      map.columns[1].unit = trend.type.unit;
    }

    while(trend.hasNext()) {
      var value = trend.next();
      map.rows.push([value.timestamp.toISOString(), value.value]);
    }
...
function TimeRange(from, to) {
  ...
  returned.toString = function() {
    return returned.to - returned.from;
  };
  return returned;
}
...
ValueTrend.prototype.toMap = function() {
  return _ValueTrend.toMap(this);
};
...
RollupTrend.prototype.toMap = function() {
  return _ValueTrend.toMap(this);
};
